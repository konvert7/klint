...
pub fn scan_prefer_nullish_coalescing_assign(
    ...
    Ok(scan_prefer_nullish_coalescing_assign_from_tree(
        tree.root_node(),
        content.as_bytes(),
    ))
}

pub(crate) fn scan_prefer_nullish_coalescing_assign_from_tree(
    root: Node<'_>,
    source: &[u8],
) -> Vec<PreferNullishCoalescingAssignRecord> {
    let mut records = Vec::new();
    walk_prefer_nullish_coalescing_assign(root, source, &mut records);
    records
}
