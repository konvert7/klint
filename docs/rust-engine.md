...
| `arch.maxCommentDensity` / `arch.maxCommentBlock` | Supported in every language the engine scans, including `countDocComments` and `ignore`. `ignore` reuses the same literal/`re:` line matcher as `forbidden`, tests the physical source line, keeps ignored lines in the density denominator, and treats them as run-connecting but non-counting for block height — identical to the TS engine, pinned by golden parity. |
