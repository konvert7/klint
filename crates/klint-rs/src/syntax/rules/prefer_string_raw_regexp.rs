...
pub fn scan_prefer_string_raw_regexp(
    ...
    Ok(scan_prefer_string_raw_regexp_from_tree(
        tree.root_node(),
        content.as_bytes(),
    ))
}

pub(crate) fn scan_prefer_string_raw_regexp_from_tree(
    root: Node<'_>,
    source: &[u8],
) -> Vec<PreferStringRawRegexpRecord> {
    let mut records = Vec::new();
    walk_prefer_string_raw_regexp(root, source, &mut records);
    records
}
